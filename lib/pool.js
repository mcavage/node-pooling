...
function createDTraceProbes(name) {
        ...
        var probes = {
                ...
                createStart: dtp.addProbe('create-start', 'int', 'json'),

                // "ok" tacked at end
                createDone: dtp.addProbe('create-done', 'int', 'json', 'int'),
        ...
        this.create(function createCallback(err, client) {
                ...
                if (err) {
                        self.probes.createDone.fire(function () {
                                return ([nextId, self._state(true), 0]);
                        });
                        callback(err);
                        return;
                }
                ...
                self.probes.createDone.fire(function () {
                        return ([nextId, self._state(true), 1]);
                });
...
Pool.prototype.shutdown = function shutdown(callback) {
        ...
        self.probes.shutdown.fire(function () {
                var out = this.resources.length - this.available.length;
